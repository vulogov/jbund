...
pub fn setloglevel(c: &cmd::Cli) {
    let env = Env::default().filter_or("JBUND_LOG_LEVEL", "error");
    match c.debug {
        0 => {
        ...
        1 => {
            let env = Env::default()
                .filter_or("JBUND_LOG_LEVEL", "jbund=info");
            env_logger::init_from_env(env);
            log::debug!("Set loglevel=info");
        }
        2 => {
            let env = Env::default()
                .filter_or("JBUND_LOG_LEVEL", "jbund=debug");
            env_logger::init_from_env(env);
            log::debug!("Set loglevel=debug");
        }
        _ => {
            let env = Env::default()
                .filter_or("JBUND_LOG_LEVEL", "jbund=trace");
            env_logger::init_from_env(env);
